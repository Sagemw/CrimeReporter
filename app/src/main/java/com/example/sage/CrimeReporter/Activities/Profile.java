...
import com.example.sage.CrimeReporter.Model.SignUpModel;
...
public class Profile extends AppCompatActivity {
    ...
    private DatabaseReference crimeReported;
    private StorageReference userPics;
    private static final int GALLERY_REQUEST =78;
    private String uid;
    private FirebaseUser user;
    private String userId;
    private String noOfCrime;
    private int totalCrimeReported;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        crimeReported = FirebaseDatabase.getInstance().getReference().child("ReportCrime");

        crimeReported.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                totalCrimeReported =0;
                for (DataSnapshot ds :dataSnapshot.getChildren()){
                    ReportingModel model = ds.getValue(ReportingModel.class);
                    userId = model.getUid();
                    Log.d("userid",""+userId);
                    if (userId.matches(uid)){
                        noOfCrime =model.getNoOfCrime();
                        totalCrimeReported = totalCrimeReported +Integer.parseInt(noOfCrime);
                    }

                    Log.d("noofcrime",""+ noOfCrime);
                }
        ...
        userRef.child(uid).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                tvuserPoints.setText(""+ totalCrimeReported);
